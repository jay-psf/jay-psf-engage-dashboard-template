set -euo pipefail
echo "== Patch 34b: regrava forgot-password + deps do RoleSwitch =="

# 1) Regrava a página /forgot-password com Client Component e export default
...
# 2) RoleSwitch: inclui onChange nas dependências do useEffect p/ remover warning
if test -f components/ui/RoleSwitch.tsx; then
  sed -i 's/useEffect(() => {[^)]*}, \[\]);/useEffect(() => { if (onChange) onChange(localRole); }, [localRole, onChange]);/' components/ui/RoleSwitch.tsx || true
fi
